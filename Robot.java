...
public class Robot extends TimedRobot {
  ...
  public double[] speedcontrol2(int i) {
    ...
    // if (controllerang.getPositionError() < -180) {
    // double err = controllerang.getSetpoint();
    // controllerang.setSetpoint(err + 360);
    // } else if (controllerang.getPositionError() > 180) {
    // double err = controllerang.getSetpoint();
    // controllerang.setSetpoint(err - 360);
    // // Trans_Lmot = controller.calculate(distance, x[i]) +
    // // controllerang.calculate(angle, y[i]);
    // // Trans_Rmot = controller.calculate(distance, x[i]) -
    // // controllerang.calculate(angle, y[i]);
    // }
    Trans_Lmot = controller.calculate(distance, x[i]) + controllerang.calculate(angle, angletotake);
    Trans_Rmot = controller.calculate(distance, x[i]) - controllerang.calculate(angle, angletotake);

    if (Math.abs(Trans_Lmot) > 0.1 || Math.abs(Trans_Rmot) > 0.1) {
      if (Math.abs(Trans_Lmot) > Math.abs(Trans_Rmot)) {
        Trans_Rmot = 0.1 * Trans_Rmot / Math.abs(Trans_Lmot);
        Trans_Lmot = 0.1 * Trans_Lmot / Math.abs(Trans_Lmot);
        // MathUtil.clamp(Trans_Lmot, -0.2, 0.2);
      } else {
        Trans_Lmot = 0.1 * Trans_Lmot / Math.abs(Trans_Rmot);
        Trans_Rmot = 0.1 * Trans_Rmot / Math.abs(Trans_Rmot);
  ...
  public void stepwisexy() {
    double totalDistance = (FL_encoder.getPosition() * (Math.PI * Units.inchesToMeters(6)) / 7.31)
        + ((BR_encoder.getPosition() * (Math.PI * Units.inchesToMeters(6))) / 7.31);
    double distance = totalDistance / 2;

    SmartDashboard.putNumber("stepwisexy() Angle", getHeading().getDegrees());
    ...
    BL.set(speedcontrol2(0)[1]);

    SmartDashboard.putNumber("angletotake", angletotake);
